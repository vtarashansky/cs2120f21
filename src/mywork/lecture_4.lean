...
/-
VOCAB
      Intro rule - is a rule that creates a proof
      Elimination rule - is a rule that uses a proof
...
-/
axiom eq_subst :  ----Is an elimination rule
...
def eq_symm : Prop :=  --This is a conjecture -- proposition believed true
  ∀ (T : Type)  -- Object T type Type
    (x y : T),  
    x = y →    -- Goal is to rewrite so you apply the eq_subst
    y = x 
